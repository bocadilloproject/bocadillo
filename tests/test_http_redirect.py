...
def test_redirect_by_route_name(app: App):
...
def test_if_redirect_to_non_existing_route_then_404(app: App):
    _setup_views_with_redirect(app, name="about")
    response = app.client.get("/")
    assert response.status_code == 404


def test_redirection_is_temporary_by_default(app: App):
    _setup_views_with_redirect(app, name="home")
    response = app.client.get("/", allow_redirects=False)
    assert response.status_code == 302


def test_permanent_redirect(app: App):
...
def test_redirect_to_internal_url(app: App):
    @app.route("/about/{who}")
...
def test_if_redirect_to_non_matching_internal_url_then_404(app: App):
    @app.route("/")
    async def index(req, res):
        app.redirect(url="/about/me")
...
def test_redirect_to_external_url(app: App):
